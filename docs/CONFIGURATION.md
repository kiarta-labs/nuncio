...
| `NUNCIO_FLAP_THRESHOLD` | `0` | Number of alternating problem/ok deliveries for the same alert fingerprint within `NUNCIO_FLAP_WINDOW_S` before flap suppression kicks in. `0` disables the feature entirely -- nothing is ever suppressed. Grouping is by fingerprint (`nuncio.fingerprint`), never by rendered text. |
| `NUNCIO_FLAP_WINDOW_S` | `10800` (3h) | Backward window the flap threshold is counted over. |
| `NUNCIO_FLAP_COOLDOWN_S` | `3600` (1h) | Once flapping is detected, the triggering alert is still delivered (with a "(flapping — Nth cycle ...)" note appended); every subsequent same-fingerprint alert is instead persisted with `outcome=suppressed_flap` (never delivered, but auditable on the dashboard) for this long. |
...
| `NUNCIO_LOGS_FIELD` | `"message"` | `openobserve` only: the stream's text column to match the alert's host/unit against (e.g. `message`, or `log` on some setups). Not used by `loki`. Must be a plain identifier (`[A-Za-z_][A-Za-z0-9_]*`); anything else falls back to `message` rather than being sent to the query. |
| `NUNCIO_LOGS_UNIT_FIELD` | `"container_name"` | `openobserve` only: a structured label column to filter the alert's unit/service against directly (exact substring match via `str_match_ignore_case`), in addition to -- never instead of -- the message-text match. Empty string disables it, falling back to message-only matching for both host and unit. Must be a plain identifier or empty; any other invalid value disables it (no fallback column is assumed). |
| `NUNCIO_CONTAINERS` | `null` | Container-state backend: `null` or `docker`. |
| `NUNCIO_DOCKER_HOST` | `unix:///var/run/docker.sock` | Docker/Podman Engine API endpoint: a unix socket (`unix:///path/to.sock`) or a TCP endpoint (`http://host:port`). Only ever calls `GET /containers/<name>/json` and `GET /containers/<name>/logs` — never a write, exec, attach, or lifecycle endpoint. **A raw Docker socket is root-equivalent to anything that can reach it** (it can create privileged containers, mount the host filesystem, etc.); a read-only socket-proxy that only forwards GET requests is strongly recommended over mounting the real socket directly, especially beyond a single-operator setup. **Proxy hardening (required if you use one):** a section-based proxy that merely allows the `containers` namespace (e.g. `tecnativa/docker-socket-proxy` with `CONTAINERS=1`) still forwards GET-only filesystem reads (`/containers/<id>/archive`, `/containers/<id>/export`) and process listings (`/containers/<id>/top`) — those endpoints let a compromised consumer read any container's filesystem or process list even though `POST` is disabled. Add explicit deny rules for `archive`/`export`/`top` ahead of the containers allow (upstream issue tecnativa/docker-socket-proxy#182), keep `POST` disabled, and verify with a probe that `GET …/archive` and `GET …/export` return 403. Nuncio needs exactly `GET /containers/<name>/json` and `GET /containers/<name>/logs` — deny everything else. |
...
**`NUNCIO_REDACT_EXTRA_RULES`** is a settings-only key with no environment-variable equivalent: a live-editable JSON list of `{"type", "regex"}` extra redaction rules, additive on top of the built-in catalog and any `NUNCIO_REDACT_EXTRA` file rules (neither can be disabled or removed through it).

**`NUNCIO_REDACT_ALLOW_KEYWORDS`** is the same kind of settings-only key, and the one setting in the whole system that *weakens* redaction rather than strengthening it. It is a live-editable JSON list of case-sensitive keywords that exempts specific tokens from the entropy backstop (the heuristic that redacts a long, high-variety, no-known-prefix token as `high_entropy` — see below) so that legitimate high-entropy-looking identifiers aren't stripped from alert text. A token is exempt iff, after splitting it on `-`, `.`, or `_`, one of the resulting segments is an *exact*, case-sensitive match for one of the configured keywords — a plain substring match is deliberately not used, since it would let a keyword ride through as a false exemption inside an unrelated, genuinely random secret. For example, with `["USW", "PoE", "U6"]` configured, a UniFi device name like `USW-Pro-Max-48-PoE-Gen2` or `U6-Enterprise` survives untouched, while a bare random secret that merely happens to *contain* the substring `U6` (without it being a delimited segment) is still redacted. The exemption applies **only** to the entropy backstop: it can never rescue a value already caught by a named pattern (API keys, JWTs, `password=`/`token=`-shaped key-value pairs, etc.) — those are replaced with their typed `«REDACTED:...»` placeholder earlier in the pipeline, before the entropy pass ever runs, so there is nothing left for the allowlist to act on. Every exemption is recorded as an `entropy_exempt` finding (count only, never the token value) so the decision is auditable; because an exemption is the *inverse* of a redaction, it is deliberately excluded from the redaction_count that feeds the dashboard's "Redaction findings" stat.
