...
# --- per-entry fault isolation: one malformed entry must never abort the
# whole batch (server.py maps any parse() exception to a 400 that also
# drops the well-formed siblings in the same POST -- see nuncio/sources/__init__.py
# SourceAdapter._fallback_parsed_alert) -----------------------------------

def _good_entry(fingerprint="ok1"):
    return {
        "status": "firing",
        "labels": {"alertname": "HighCPU", "instance": "web-1", "severity": "critical"},
        "annotations": {"summary": "CPU above 90% for 5m"},
...
def test_one_malformed_entry_does_not_abort_batch():
    payload = {
        "receiver": "nuncio", "status": "firing", "commonLabels": {},
        "alerts": [_good_entry("ok1"), "not-a-dict", _good_entry("ok2")],
    }
    out = Grafana().parse(payload, headers={})
    assert len(out) == 3
    assert out[0].alert["host"] == "web-1" and out[0].alert["severity"] == "critical"
    assert out[2].alert["host"] == "web-1" and out[2].alert["severity"] == "critical"
    assert out[1].alert["severity"] == "unknown"
    assert "[UNPARSEABLE]" in out[1].raw_text


def test_list_valued_labels_entry_degrades_to_raw_not_crash():
    payload = {
        "receiver": "nuncio", "status": "firing", "commonLabels": {},
        "alerts": [{"status": "firing", "labels": ["a", "b"], "annotations": {}}],
    }
    out = Grafana().parse(payload, headers={})
    assert len(out) == 1
    assert out[0].alert["severity"] == "unknown"
    assert "[UNPARSEABLE]" in out[0].raw_text


def test_malformed_entry_key_is_stable_across_retries():
    payload = {
        "receiver": "nuncio", "status": "firing", "commonLabels": {},
        "alerts": ["not-a-dict"],
    }
    out1 = Grafana().parse(payload, headers={})
    out2 = Grafana().parse(payload, headers={})
    assert out1[0].key == out2[0].key


def test_all_malformed_batch_still_yields_alerts():
    payload = {
        "receiver": "nuncio", "status": "firing", "commonLabels": {},
        "alerts": [42, None],
    }
    out = Grafana().parse(payload, headers={})
    assert len(out) == 2
    assert all("[UNPARSEABLE]" in pa.raw_text for pa in out)


def test_structural_guard_still_rejects_missing_alerts():
    with pytest.raises(ValueError):
        Grafana().parse({"status": "firing", "alerts": []}, headers={})
    with pytest.raises(ValueError):
        Grafana().parse({"status": "firing", "alerts": "nope"}, headers={})
