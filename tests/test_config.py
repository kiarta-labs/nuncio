...
# --- structural secret masking: SECRET_KEYS is masked by identity, never by
# dogfooding redact() -- a value whose key is a known credential must never
# reach the redactor's value-shape heuristics, which can miss a credential
# nested in a low-entropy shape (e.g. JSON headers). ---

def test_masked_config_hides_llm_headers_even_when_low_entropy():
    # A deliberately low-entropy bearer token: the "auth_header" redactor
    # rule is line-anchored ("^Authorization:") and misses this because it's
    # nested inside a JSON blob, not a standalone header line.
    s = config.load_settings(base_env(NUNCIO_LLM_HEADERS='{"Authorization":"Bearer abc12345"}'))
    masked = config.masked_config_dict(s)
    assert "abc12345" not in json.dumps(masked)
    assert "Bearer" not in json.dumps(masked)
    assert masked["NUNCIO_LLM_HEADERS"] == "«set»"


def test_masked_config_hides_webhook_headers_even_when_low_entropy():
    s = config.load_settings(base_env(NUNCIO_WEBHOOK_HEADERS='{"Authorization":"Bearer abc12345"}'))
    masked = config.masked_config_dict(s)
    assert "abc12345" not in json.dumps(masked)
    assert masked["NUNCIO_WEBHOOK_HEADERS"] == "«set»"


def test_masked_config_every_secret_key_renders_set_or_unset():
    s = config.load_settings(base_env(
        NUNCIO_LLM_KEY="a-key", NUNCIO_KNOWLEDGE_KEY="", NUNCIO_ASSIST_KEY="a-key",
        NUNCIO_ADMIN_TOKEN="a-token", NUNCIO_INGEST_TOKEN="a-token",
    ))
    masked = config.masked_config_dict(s)
    for name in config.SECRET_KEYS:
        assert masked[name] in ("«set»", "«unset»"), (name, masked[name])


def test_masked_config_unset_secret_shows_unset_not_empty_string():
    s = config.load_settings(base_env())
    masked = config.masked_config_dict(s)
    assert masked["NUNCIO_LLM_KEY"] == "«unset»"


def test_masked_config_ntfy_url_basic_auth_still_dogfoods_redact():
    # Regression: NUNCIO_NTFY_URL is NOT in SECRET_KEYS -- it must keep going
    # through the normal per-line redact() dogfood path (the basic_auth rule
    # catches embedded creds in otherwise-non-secret URLs).
    s = config.load_settings(base_env(NUNCIO_NTFY_URL="https://u:hunter2@host/"))
    masked = config.masked_config_dict(s)
    assert "hunter2" not in masked["NUNCIO_NTFY_URL"]
    assert masked["NUNCIO_NTFY_URL"] != "«set»"  # dogfooded redaction, not structural masking


def test_config_json_body_never_leaks_a_secret_key_value(tmp_path):
    s_env = base_env(NUNCIO_DATA_DIR=str(tmp_path), NUNCIO_LLM_HEADERS='{"Authorization":"Bearer abc12345"}',
                      NUNCIO_ASSIST_KEY="hunter2hunter2")
    app, settings = config.build_app(config.load_settings(s_env))
    try:
        assert b"abc12345" not in app.config_json
        assert b"hunter2hunter2" not in app.config_json
    finally:
        app.store.close()
...
def test_build_log_client_defaults_unit_field_to_container_name():
    s = config.load_settings(base_env(NUNCIO_LOGS="openobserve", NUNCIO_LOGS_URL="http://o2:5080/api/default"))
    client = config.build_log_client(s)
    assert client._unit_field == "container_name"


def test_build_log_client_wires_logs_unit_field_through_to_openobserve():
    s = config.load_settings(base_env(NUNCIO_LOGS="openobserve", NUNCIO_LOGS_URL="http://o2:5080/api/default",
                                       NUNCIO_LOGS_UNIT_FIELD=""))
    client = config.build_log_client(s)
    assert client._unit_field == ""
...
def test_redaction_extra_rules_bad_regex_attributed_to_its_own_key(tmp_path):
    # M-1: compile_extra_rules() raising a bad-regex ValueError must be
    # attributed to NUNCIO_REDACT_EXTRA_RULES, not bucketed under the
    # placeholder "_" key -- the settings UI highlights a row by key, and
    # "_" matches no row, so an un-attributed error silently highlights
    # nothing.
    app, settings = _app_with_data_dir(tmp_path)
    with pytest.raises(config.SettingsValidationError) as exc_info:
        config.apply_changes(app, {"NUNCIO_REDACT_EXTRA_RULES": [{"type": "bad", "regex": "("}]})
    assert "NUNCIO_REDACT_EXTRA_RULES" in exc_info.value.errors
    assert "_" not in exc_info.value.errors
    app.store.close()
