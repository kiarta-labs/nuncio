...
def test_post_settings_413_body_too_large_but_unauthenticated_returns_auth_failure(tmp_path):
    # M-2: auth must be checked strictly BEFORE any other 4xx from this
    # handler, including the request-size guard -- an unauthenticated
    # over-large POST must 401/403 (no token / wrong token), not leak a 413
    # ahead of the auth check.
    app, settings = build(tmp_path, NUNCIO_ADMIN_TOKEN="tok")
    huge = b'{"set": {"NUNCIO_DELIVERY_TITLE": "' + b"x" * (settings_ui.MAX_BODY_BYTES + 10) + b'"}}'
    status, body = settings_ui.handle_post(app, huge, _Headers({"X-Admin-Token": "wrong"}))
    assert status == 401
    app.store.close()
...
def test_settings_json_assist_key_is_masked_even_though_never_editable(tmp_path):
    # NUNCIO_ASSIST_KEY is a NEVER-key (env-only) that must render masked on
    # GET /settings.json exactly like NUNCIO_LLM_KEY/NUNCIO_KNOWLEDGE_KEY --
    # it was previously omitted from the local _NEVER_SECRETS set here.
    app, settings = build(tmp_path, NUNCIO_ASSIST_ENABLED="true",
                           NUNCIO_ASSIST_URL="http://assist.example",
                           NUNCIO_ASSIST_KEY="hunter2hunter2")
    body = settings_ui.render_settings_json(app)
    assert b"hunter2hunter2" not in body
    parsed = json.loads(body)
    assert parsed["keys"]["NUNCIO_ASSIST_KEY"]["secret"] is True
    assert parsed["keys"]["NUNCIO_ASSIST_KEY"]["value"] == "«set»"
    app.store.close()


def test_settings_json_secret_keys_come_from_config_single_source_of_truth(tmp_path):
    # settings.py must derive "secret" from config.SECRET_KEYS, not a local
    # copy -- assert every SECRET_KEYS member renders masked on both the
    # UI-editable and NEVER-key render loops.
    app, settings = build(tmp_path)
    parsed = json.loads(settings_ui.render_settings_json(app))
    for name in config.SECRET_KEYS:
        assert parsed["keys"][name]["secret"] is True, name
...
def test_page_js_load_sources_counts_from_stats_by_source_24h_not_ingested_by_source(tmp_path):
    # I-1: the per-source "N ingested (24h)" count rendered by loadSources()
    # must come from STATS.by_source_24h -- a real persistent 24h window
    # computed from alerts.db (dashboard.py's render_stats_json) -- and NOT
    # from /sources' ingested_by_source, which is an in-memory counter that
    # resets on every process restart and isn't actually a 24h window. The
    # `registered` adapter list itself still comes from SOURCES/`/sources`.
    app, settings = build(tmp_path)
    html = settings_ui.render_settings_html(app).decode()
    fn = html[html.index("async function loadSources()"):]
    fn = fn[:fn.index("\n}\n") + 3]
    assert "by_source_24h" in fn
...
def test_form_js_send_apply_surfaces_underscore_error_message_not_generic_toast(tmp_path):
    # M-1: cross-field/rebuild errors the server can't attribute to a
    # specific key come back keyed "_" (e.g. a bad NUNCIO_REDACT_EXTRA_RULES
    # regex, or a startup ConfigError). "_" matches no `[data-k="_"]` row, so
    # showRowErrors() highlights nothing for it -- sendApply's rejected-apply
    # branch must not then toast the generic "see the highlighted fields"
    # promise; it must fall back to the "_" entry's own message text.
    from nuncio.web.forms import FORM_JS
    fn = FORM_JS[FORM_JS.index("async function sendApply("):]
    fn = fn[:fn.index("\n}\n") + 3]
    assert "errors['_']" in fn or 'errors["_"]' in fn or "errors._" in fn
...
def test_group_rows_key_name_span_is_html_escaped(tmp_path):
    # M-3: groupRows() prints the raw key name into a visible
    # <span class="k">...</span> alongside the escaped label. Keys come from
    # the server schema today (not exploitable in practice), but it's the
    # lone un-escaped interpolation in the row-rendering source -- wrap it in
    # the same esc() helper used for every other dynamic value on the row.
    # (data-k="' + k + '" and the onclick(...'+k+'...) handler args are left
    # alone: those are selector/JS-argument positions esc()'s HTML-entity
    # output would corrupt, not rendered visible text/markup.)
    from nuncio.web.forms import FORM_JS
    fn = FORM_JS[FORM_JS.index("function groupRows("):]
    fn = fn[:fn.index("\n}\n") + 3]
    assert "<span class=\"k\">' + esc(k) + '</span>" in fn
