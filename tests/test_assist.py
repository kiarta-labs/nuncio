...
class FakeAssistClient:
    ...
    def insight(self, payload, timeout=None):
...
def test_assist_client_insight_passes_timeout_kwarg_through():
    # MUST-FIX-style timeout threading: without this, a non-streaming
    # response hangs at the LLM client's fixed construction-time timeout
    # regardless of the caller's own per-call bound.
    calls = []

    class RecordingLLM:
        def enrich(self, messages, max_tokens=400, response_format=None, timeout=None):
            calls.append(timeout)
            return "root cause: disk full"

    from nuncio.redactor import scrub_for_assist_plane
    payload = scrub_for_assist_plane("disk full on host01")
    client = AssistClient(RecordingLLM())
    client.insight(payload, timeout=12.5)
    assert calls == [12.5]
...
def test_process_item_threads_the_remaining_deadline_as_client_timeout(store):
    # MUST-FIX-style timeout threading: _process_item must pass its own
    # per-call bound (deadline.remaining()) through to client.insight() as
    # the client's timeout, not leave the client at whatever fixed timeout
    # it was constructed with.
    store.persist("k1", RAW, severity="critical")
    assist_client = FakeAssistClient([("ok", "root cause: disk exhaustion")])
    dispatch = FakeDispatch()
    fake_clock = FakeClock()  # static -- deadline.remaining() stays exactly timeout_s
    track = make_track(assist_client, dispatch, store, timeout_s=5.0, clock=fake_clock)
    envelope = Envelope(severity="critical", host="h", service="s", headline="hl", summary="s", detail="d")

    track._process_item("k1", envelope, "ctx", False)

    assert len(assist_client.timeouts) == 1
    assert assist_client.timeouts[0] == pytest.approx(5.0)
