...
# whole batch (see nuncio/sources/__init__.py SourceAdapter._fallback_parsed_alert) --
...
def test_one_malformed_entry_does_not_abort_batch():
    payload = {"alerts": [_good_entry("ok1"), "not-a-dict", _good_entry("ok2")]}
    out = Alertmanager().parse(payload, headers={})
    assert len(out) == 3
    assert out[0].alert["host"] == "10.0.0.5" and out[0].alert["severity"] == "warning"
    assert out[2].alert["host"] == "10.0.0.5" and out[2].alert["severity"] == "warning"
...
def test_list_valued_labels_entry_degrades_to_raw_not_crash():
    payload = {"alerts": [{"status": "firing", "labels": ["a", "b"], "annotations": {}}]}
    out = Alertmanager().parse(payload, headers={})
    assert len(out) == 1
    assert out[0].alert["severity"] == "unknown"
...
def test_malformed_entry_key_is_stable_across_retries():
    payload = {"alerts": ["not-a-dict"]}
    out1 = Alertmanager().parse(payload, headers={})
    out2 = Alertmanager().parse(payload, headers={})
    assert out1[0].key == out2[0].key


def test_all_malformed_batch_still_yields_alerts():
    payload = {"alerts": [42, None]}
    out = Alertmanager().parse(payload, headers={})
    assert len(out) == 2
...
def test_structural_guard_still_rejects_missing_alerts():
    with pytest.raises(ValueError):
        Alertmanager().parse({"alerts": []}, headers={})
    with pytest.raises(ValueError):
        Alertmanager().parse({"alerts": "nope"}, headers={})
