...
# --- concurrent-access resilience: a busy_timeout so a transient lock from a
# concurrent external reader session degrades to a short wait, never an
# immediate "database is locked" error. ---

def test_busy_timeout_pragma_is_set_on_a_fresh_store(store):
    (busy_timeout,) = store._conn.execute("PRAGMA busy_timeout").fetchone()
    assert busy_timeout == 5000


def test_busy_timeout_pragma_set_explicitly_before_the_wal_pragma():
    # Python's sqlite3.connect() default `timeout=5.0` happens to already
    # yield the same 5000ms busy_timeout read-back above, so that assertion
    # alone can't tell an explicit PRAGMA apart from incidental defaults.
    # This is a structural guard for the actual requirement: an EXPLICIT
    # "PRAGMA busy_timeout=5000" immediately after connect() and before the
    # journal_mode=WAL pragma (a mode change can itself hit BUSY), so the
    # behavior no longer depends on connect()'s default staying 5.0.
    import inspect
    from nuncio import store as store_mod
    src = inspect.getsource(store_mod.Store.__init__)
    busy_idx = src.find("busy_timeout")
    wal_idx = src.find("journal_mode=WAL")
    assert busy_idx != -1, "no explicit PRAGMA busy_timeout statement in Store.__init__"
    assert wal_idx != -1
