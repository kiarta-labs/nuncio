...
def test_send_rejects_non_http_scheme(monkeypatch):
    ...
    monkeypatch.setattr("nuncio.delivery.slack.urllib.request.urlopen", fail_urlopen)
    s = Slack({"webhook_url": "file:///tmp/x"})  # real default transport
    try:
        assert not s.send("t", "b")
    except ValueError:
