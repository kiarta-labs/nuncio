...
def _escape_logql_re(value):
    # For the `{label=~".*host.*"}` LogQL stream-selector matcher: a REGEX
    # context, same treatment as _escape_promql (see clients/metrics.py) --
    # re.escape() neutralizes the value as a regex first, then the quote is
    # string-literal-escaped. Backslash BEFORE quote, same ordering
    # rationale: the escaper's own backslashes must be string-escaped too or
    # a value containing a backslash would double-escape and the
    # literal-closing quote could still break out.
    return re.escape(str(value)).replace("\\", "\\\\").replace('"', '\\"')


def _escape_logql(value):
    # For the `|= "unit"` line filter: a plain SUBSTRING match, NOT a regex
    # matcher -- re.escape() here would be wrong (it would make the operator
    # search for the literal regex-metacharacter-escaped text, which isn't
    # what a substring filter means). Only the string-literal escaping
    # applies: backslash before quote, same ordering rationale as above.
    s = str(value)
...
class LokiClient:
    ...
    def _query(self, host, unit, window_s):
        ...
        logql = f'{{{self._label}=~".*{_escape_logql_re(host)}.*"}}'
        if unit:
