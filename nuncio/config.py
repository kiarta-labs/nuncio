...
UI_EDITABLE = {
    ...
    "NUNCIO_REDACT_ALLOW_KEYWORDS": KeySpec(category="live", type="json", default=[], group="redaction", confirm=True,
                                            label="Entropy-backstop allow-keywords",
                                            help="JSON list of case-sensitive keywords. A token the entropy "
                                                 "backstop would redact is spared if any of its -/./_-delimited "
                                                 "segments exactly equals a keyword. Exempts ONLY the high-entropy "
                                                 "heuristic -- never a named-pattern secret (API keys, JWTs, "
                                                 "key=value secrets). This is the only setting that WEAKENS "
                                                 "redaction."),
...
# --- structural secret masking: the single source of truth for "is this
# NUNCIO_* name a credential", used by BOTH masked_config_dict (below) and
# nuncio.web.settings's GET renderers. Anything in here is masked by
# IDENTITY (the key name), never by dogfooding the redactor's value-shape
# heuristics -- a credential nested in a low-entropy shape (e.g. a bearer
# token inside a JSON headers blob) can slip past redact()'s line-anchored
# rules, but can never slip past a key-name check. Built from every
# UI_EDITABLE key already marked secret=True, plus the handful of NEVER-key
# credentials that have no KeySpec at all (so no `.secret` flag to read).
SECRET_KEYS = frozenset(
    k for k, s in UI_EDITABLE.items() if getattr(s, "secret", False)
) | {
...
class Settings:
    ...
    def __init__(self, env, overrides=None):
        ...
        self.NUNCIO_REDACT_ALLOW_KEYWORDS = list(UI_EDITABLE["NUNCIO_REDACT_ALLOW_KEYWORDS"].default)
        self.source["NUNCIO_REDACT_ALLOW_KEYWORDS"] = "default"
...
def apply_changes(app, set_map, reset_list=None):
    ...
    with _APPLY_LOCK:
        ...
        if "NUNCIO_REDACT_EXTRA_RULES" in live_changed:
            # Attributed to its own key (not the "_" placeholder) so the
            # settings UI can highlight the offending row -- "_" matches no
            # row and would leave the toast pointing at nothing.
            try:
                rebuilt["extra_rules"] = compile_extra_rules(candidate.NUNCIO_REDACT_EXTRA_RULES)
            except (ConfigError, ValueError) as e:
                raise SettingsValidationError({"NUNCIO_REDACT_EXTRA_RULES": str(e)})
        if "NUNCIO_REDACT_ALLOW_KEYWORDS" in live_changed:
            try:
                rebuilt["allow_keywords"] = compile_allow_keywords(candidate.NUNCIO_REDACT_ALLOW_KEYWORDS)
            except (ConfigError, ValueError) as e:
                raise SettingsValidationError({"NUNCIO_REDACT_ALLOW_KEYWORDS": str(e)})
...
def masked_config_dict(settings):
    """The effective config, masked for `/config.json` (unauthenticated) and
    the startup log. Two independent layers, deliberately not one:

    - SECRET_KEYS (a known-credential NAME) is masked STRUCTURALLY -- always
      the literal `«set»`/`«unset»`, never the value, never passed through
      the redactor. A key-name check can't be fooled by a value shape the
      redactor's heuristics happen to miss (e.g. a bearer token nested in a
      low-entropy JSON headers blob).
    - every OTHER key is still redacted by nuncio.redactor.redact() itself --
      the same function that protects every outbound alert payload, so a
      non-secret-flagged value that still embeds a credential (e.g. basic-auth
      creds in a collector URL) is caught the same way it would be in normal
      traffic.
    """
...
def build_app(settings=None, clock=None):
    ...
    set_allow_keywords(settings.NUNCIO_REDACT_ALLOW_KEYWORDS)
