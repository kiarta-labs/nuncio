...
class AssistClient:
    ...
    def insight(self, payload, timeout=None):
        ...
        # MUST-FIX-style timeout threading (mirrors Engine._call_bounded): the
        # caller's own per-call bound must also become the HTTP socket
        # timeout -- a non-streaming completion sends nothing until
        # generation finishes, so without this the socket would sit at the
        # client's fixed construction-time timeout regardless of a
        # deliberately larger/smaller bound passed in here.
        raw = self._llm.enrich(_assist_messages(payload.text), max_tokens=200, timeout=timeout)
