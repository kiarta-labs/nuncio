...
# Secrets that live OUTSIDE the UI_EDITABLE table (the NEVER-keys) but must
# still render masked on GET for the same reason every other secret does.
_NEVER_SECRETS = frozenset({"NUNCIO_LLM_KEY", "NUNCIO_KNOWLEDGE_KEY", "NUNCIO_ADMIN_TOKEN"})
...
def build_settings_json(app):
    ...
    if settings is not None:
        for name, spec in _cfg().UI_EDITABLE.items():
            ...
            keys[name] = {
                "value": _mask(value) if spec.secret else _redacted(value),
...
async function loadSources() {
  ...
  try {
    ...
    const counts = data.ingested_by_source || {};
