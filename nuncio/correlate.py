...
CAUSAL-ENTITY relation to the alert being enriched:
  ...
  (c) a declared `dependency_hints` edge, gated in BOTH directions: the
      candidate row is a declared UPSTREAM of the alert's service (e.g.
      alert on "infisical", row is "infisical-postgres") OR a declared
      DEPENDENT of it (e.g. alert on "infisical-postgres", row is
      "infisical", which declares a dependency on it). Either direction is a
      genuine causal-entity relation between the two services; only the
      rendered reason differs ("upstream dependency of X" vs. "declared
      dependent of X").
...
def rank_correlated(rows, alert, tokens=(), now=0.0, window_s=600, top_n=8, deps=None, host_domains=()):
    ...
    try:
        ...
        # Bidirectional extension: services that declare a dependency ON the
        # alert's own service (i.e. the alert's service is one of THEIR
        # upstreams) -- the other end of the same causal edge. Guarded by
        # a_service/a_unit being non-None so an unresolvable alert service
        # never fuzzy-matches every deps entry.
        downstreams = []
        if deps and (a_service or a_unit):
            for dependent, ups in deps.items():
                if not dependent:
                    continue
                for u in (ups or []):
                    if not u:
                        continue
                    if (a_service and _norm(u) == a_service) or (
                            a_unit and resolve_unit_strict({"service": u}) == a_unit):
                        downstreams.append(str(dependent))
                        break
            ...
            try:
                ...
                if upstreams:
                    if r_service is not None:
                        ...
                        dep_hit_up = any(_word_re(u).search(summary) for u in upstreams)

                dep_hit_down = False
                if downstreams:
                    if r_service is not None:
                        dep_hit_down = any(
                            (r_service_norm is not None and r_service_norm == _norm(s))
                            or (r_unit is not None and r_unit == resolve_unit_strict({"service": s}))
                            for s in downstreams
                        )
                    else:  # legacy fallback
                        dep_hit_down = any(_word_re(s).search(summary) for s in downstreams)

                dep_hit = dep_hit_up or dep_hit_down
                ...
                if dep_hit_up:
                    ...
                    score += _DEP_WEIGHT
                    reasons.append(f"declared dependent of {service}")
